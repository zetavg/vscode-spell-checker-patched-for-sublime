# Spelling Checker for Visual Studio Code Patched for Sublime Text 4
...
Make an executable wrapper for the LSP server entry file, `packages/_server/dist/main.js`:

```sh
echo '#!/usr/bin/env bash' >> cspell-server
echo "node $(realpath packages/_server/dist/main.js) \"\$@\"" >> cspell-server
chmod +x cspell-server
```

Copy the `cspell-server` executable to somewhere in your `PATH`, for example, `/usr/local/bin`:

```sh
sudo cp cspell-server /usr/local/bin/
```

Test if it works (no error means it works):
...
Open LSP Settings in Sublime Text (Packages/User/LSP.sublime-settings), add the following under "clients":
...
```
    "cspell": {
      ...
      "settings": {
          ...
          "cSpell.suggestionsTimeout": 4000,
...
-   [Serbian](https://marketplace.visualstudio.com/items?itemName=streetsidesoftware.code-spell-checker-serbian)
